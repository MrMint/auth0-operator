...
{
    public abstract class V1TenantEntityController<TEntity, TSpec, TStatus, TConf>
        : V1Controller<TEntity, TSpec, TStatus, TConf>
        ...
        where TConf : class
    {
        ...
        /// Using AsyncLocal to ensure thread-safety in concurrent scenarios.
        /// </summary>
        readonly AsyncLocal<TenantApiContext?> _currentApiContext = new();

        /// <summary>
        /// Gets the current tenant API context during reconciliation.
        /// Available only during the execution of abstract methods called from Reconcile or DeletedAsync.
        /// </summary>
        protected TenantApiContext? CurrentApiContext => _currentApiContext.Value;
        ...
        /// <returns></returns>
        protected abstract Task<Hashtable?> Get(
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        protected abstract Task<string?> Find(
            IManagementApiClient api,
            TEntity entity,
            TSpec spec,
            string defaultNamespace,
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        protected abstract Task<string> Create(
            IManagementApiClient api,
            TConf conf,
            string defaultNamespace,
            CancellationToken cancellationToken
        ...
        protected override async Task Reconcile(TEntity entity, CancellationToken cancellationToken)
        {
            ...
            if (entity.Spec.Conf is null)
                throw new InvalidOperationException(
                    $"{EntityTypeName} {entity.Namespace()}/{entity.Name()} missing configuration."
                );

            var tenant = await ResolveTenantRef(
                entity.Spec.TenantRef,
                entity.Namespace(),
                cancellationToken
            );
            if (tenant is null)
            ...
            try
            {
                ...
                {
                    Logger.LogDebug(
                        "{EntityTypeName} {Namespace}/{Name} has not yet been reconciled, checking if entity exists in Auth0.",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
                    ...
                    if (entityId is null)
                    {
                        Logger.LogInformation(
                            "{EntityTypeName} {Namespace}/{Name} could not be located, creating.",
                            EntityTypeName,
                            entity.Namespace(),
                            entity.Name()
                        );
                        ...
                        {
                            Logger.LogInformation(
                                "{EntityTypeName} {Namespace}/{Name} does not support creation.",
                                EntityTypeName,
                                entity.Namespace(),
                                entity.Name()
                            );
                        ...
                        if (ValidateCreate(init) is string msg)
                            throw new InvalidOperationException(
                                $"{EntityTypeName} {entity.Namespace()}/{entity.Name()} is invalid: {msg}"
                            );

                        // create new entity and associate
                        entity.Status.Id = await Create(
                            api,
                            init,
                            entity.Namespace(),
                            cancellationToken
                        ...
                        Logger.LogInformation(
                            "{EntityTypeName} {Namespace}/{Name} created with {Id}",
                            EntityTypeName,
                            entity.Namespace(),
                            entity.Name(),
                            entity.Status.Id
                    ...
                    else
                    {
                        ...
                        Logger.LogInformation(
                            "{EntityTypeName} {Namespace}/{Name} found with {Id}",
                            EntityTypeName,
                            entity.Namespace(),
                            entity.Name(),
                            entity.Status.Id
                ...
                if (string.IsNullOrWhiteSpace(entity.Status.Id))
                    throw new InvalidOperationException(
                        $"{EntityTypeName} {entity.Namespace()}/{entity.Name()} is missing an existing ID."
                    );

                // attempt to retrieve existing entity
                Logger.LogDebug(
                    "{EntityTypeName} {Namespace}/{Name} checking if entity exists in Auth0 with ID {Id}",
                    EntityTypeName,
                    entity.Namespace(),
                    entity.Name(),
                    entity.Status.Id
                );
                var lastConf = await Get(
                    api,
                    entity.Status.Id,
                    entity.Namespace(),
                    cancellationToken
                ...
                if (lastConf is null)
                {
                    ...
                    Logger.LogInformation(
                        "{EntityTypeName} {Namespace}/{Name} not found in Auth0, clearing status and scheduling recreation",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
                    ...
                    throw new RetryException(
                        $"{EntityTypeName} {entity.Namespace()}/{entity.Name()} has missing API object, invalidating."
                    );
                }
                ...
                {
                    Logger.LogDebug(
                        "{EntityTypeName} {Namespace}/{Name} does not support update.",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        protected virtual Task ApplyStatus(
            IManagementApiClient api,
            TEntity entity,
            Hashtable lastConf,
            string defaultNamespace,
        ...
        /// <inheritdoc />
        public override sealed async Task DeletedAsync(
            TEntity entity,
            CancellationToken cancellationToken
        )
        {
            // Resolve tenant and set context before the main try-catch
            V1Tenant? tenant = null;
            if (entity.Spec.TenantRef is not null)
            {
                tenant = await ResolveTenantRef(
                    entity.Spec.TenantRef,
                    entity.Namespace(),
                    cancellationToken
                );
            ...
            try
            {
                if (entity.Spec.TenantRef is null)
                    throw new InvalidOperationException(
                        $"{EntityTypeName} {entity.Namespace()}/{entity.Name()} is missing a tenant reference."
                    );

                if (tenant is null)
                    throw new InvalidOperationException(
                        $"{EntityTypeName} {entity.Namespace()}/{entity.Name()} is missing a tenant."
                    );

                var api = _currentApiContext.Value?.Client;
                if (api is null)
                    throw new InvalidOperationException(
                        $"{EntityTypeName} {entity.Namespace()}/{entity.Name()} failed to retrieve API client."
                    );

                if (string.IsNullOrWhiteSpace(entity.Status.Id))
                {
                    Logger.LogWarning(
                        "{EntityTypeName} {EntityNamespace}/{EntityName} has no known ID, skipping delete (reason: entity was never successfully created in Auth0).",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
                ...
                {
                    Logger.LogWarning(
                        "{EntityTypeName} {EntityNamespace}/{EntityName} with ID {Id} not found in Auth0, skipping delete (reason: already deleted externally).",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name(),
                        entity.Status.Id
                ...
                {
                    Logger.LogInformation(
                        "{EntityTypeName} {Namespace}/{Name} does not support delete (reason: Delete policy not enabled).",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
                ...
                {
                    Logger.LogInformation(
                        "{EntityTypeName} {Namespace}/{Name} initiating deletion from Auth0 with ID: {Id} (reason: Kubernetes entity was deleted)",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name(),
                        entity.Status.Id
                    ...
                    Logger.LogInformation(
                        "{EntityTypeName} {Namespace}/{Name} deletion completed successfully",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
            ...
            catch (ErrorApiException e)
            {
                try
                {
                    Logger.LogError(
                        e,
                        "API error deleting {EntityTypeName} {EntityNamespace}/{EntityName}: {Message}",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name(),
                        e.ApiError?.Message
                    );
                    await DeletingWarningAsync(
                        entity,
                        "ApiError",
                        e.ApiError?.Message ?? "",
                        cancellationToken
            ...
            catch (RateLimitApiException e)
            {
                ...
                {
                    Logger.LogError(
                        "Rate limit hit deleting {EntityTypeName} {EntityNamespace}/{EntityName}",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
            ...
            catch (RetryException e)
            {
                ...
                {
                    Logger.LogError(
                        "Retry hit deleting {EntityTypeName} {EntityNamespace}/{EntityName}: {Messge}",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name(),
                        e.Message
                ...
                Logger.LogInformation(
                    "Rescheduling delete after {TimeSpan}.",
                    TimeSpan.FromMinutes(1)
                );
            ...
            catch (Exception e)
            {
                try
                {
                    Logger.LogError(
                        e,
                        "Unexpected exception deleting {EntityTypeName} {EntityNamespace}/{EntityName}.",
                        EntityTypeName,
                        entity.Namespace(),
                        entity.Name()
                    );
